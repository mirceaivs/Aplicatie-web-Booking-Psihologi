...
export function ProgramariClient() {
    ...
    useEffect(() => {
        const getProgramari = async () => {
            try {
                ...
                const response = await axios.get('http://localhost:3000/api/user/programari/client', {
        ...
        <Accordion className="border border-1 border-grey rounded" >
            ...
            {listaProgramari.length !== 0 && listaProgramari.map((programare: Programaris, index: number) => (
                <Accordion.Item eventKey={index.toString()} key={index}>
                    <Accordion.Header><h5>Programarea: {programare.data_programare} Psiholog: {programare.user_nume} {programare.user_prenume}
